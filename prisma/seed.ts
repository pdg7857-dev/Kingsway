...
async function main() {
  ...
  // ───── Tasks (mix across businesses + priorities + dates)
  const taskSeed: Array<Partial<Parameters<typeof prisma.task.create>[0]["data"]>> = [
  ...
  // ───── AI Insight: pre-seed today's briefing so the dashboard has content
  ...
  // ───── Automations (seeded, disabled by default)
