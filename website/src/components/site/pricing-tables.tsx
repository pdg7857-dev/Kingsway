...
export function PricingTables() {
  ...
  return (
    <div>
      <div className="mx-auto mb-10 flex w-fit items-center gap-1 rounded-full border border-line bg-paper-muted p-1">
  ...
  return (
    <button
      ...
      className={`rounded-full px-5 py-2 text-sm font-semibold transition ${
        active ? "bg-ink text-white" : "text-slate-200 hover:text-brand-300"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl border p-6 ${
        plan.featured
          ? "border-brand-500 bg-paper-muted shadow-lift ring-1 ring-brand-500"
          : "border-line bg-paper-muted shadow-card"
      }`}
    >
      {plan.featured && (
      ...
      <p className="mt-1 text-xs font-medium uppercase tracking-wide text-slate-400">{plan.scope}</p>
      <div className="mt-4 flex items-baseline gap-1">
        {plan.startingAt && <span className="text-sm text-slate-400">from</span>}
        <span className="text-3xl font-bold text-white">{plan.price}</span>
        <span className="text-sm text-slate-400">{plan.priceNote}</span>
      </div>
      <p className="mt-3 text-sm leading-6 text-slate-300">{plan.blurb}</p>
      ...
      {plan.footnote && (
        <p className="mt-4 rounded-md bg-paper-soft px-3 py-2 text-xs leading-snug text-slate-400">
          {plan.footnote}
        </p>
      )}
      <Link
